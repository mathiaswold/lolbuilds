...
def save(champion, item_set, version, source):
    """ Saves an item set for a champion for a given role. """

    # standard layout for an item set
    output = {
        "title": f"{champion['display_name']} {item_set['role']} {version}",
        "type": "custom",
        "map": "any",
        "mode": "any",
        "priority": False,
        "sortrank": item_set["rank"],
    ...
    item_set_path = os.path.join(
        champion_path, f"{source}_{item_set['role']}.json")
...
def delete_all():
    from sources.championgg import championgg

    """ Deletes all item sets generated by this script """
    champions = championgg.get_champions()
    ...
    for champion in champions:
        try:
            ...
            for item_set in item_sets:
                if any(["ChampionGG_" in item_set]):
